...
import { loadGLTF, poolCue } from './blenderImport.js'
import { init, setLight } from './setup.js'
import { rotateAroundWorldAxis } from './rotations.js'

import { euler, h } from './euler.js'
import { edgeDetection, ballCollision } from './collision.js'
import { userInputs } from './playerInputs.js'
...
for (let n = 1; n <= row; ++n) {
	...
	for (let m = 1; m <= nrow; ++m) {
		p.push(
			new THREE.Vector2(
				(2 / 3) * width + nrow * radius * space,
				height / 2 + (space / 2) * (nrow - 1) * radius - space * radius * (m - 1)
			)
		)
	}
}
...
// Helper function to update Force value
function updateForce(value) {
	Force = value
}

function updateForceTemp(value) {
	Force_temp = value
}
...
function render() {
	...
	for (let i = 0; i < spheres.length; ++i) {
		...
		for (let j = i + 1; j < spheres.length; ++j) {
			const ballOut = ballCollision(v[i], v[j], p[i], p[j], prevPosition[i], prevPosition[j], i, j)
			v[i] = ballOut[0]
			v[j] = ballOut[1]
			p[i] = ballOut[2]
			p[j] = ballOut[3]
		...
		if (
			...
			poolCue.position.set(
				p[0].x - (Force_temp / 1000) * Math.cos(tau[0]),
				0.01,
				p[0].y - (Force_temp / 1000) * Math.sin(tau[0])
			)
...
function animate() {
	...
	setTimeout(function () {
		// Set timeout because of the time it takes to load the billiard table, can be removed when UI exists
		render()
	}, 20)
}
...
export { scene, camera, width, height, controls, renderer, spheres }
export {
	Force,
	Force_temp,
	updateForce,
