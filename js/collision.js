...
import { radius, width, height, tau, Friction, m, g, my, initialVelocity, spheres } from './main.js'
...
function updateRowCounter(value) {
	rowCounter = value
}
...
function edgeDetection(inP, prevPos, inV, n) {
	...
	if (
		inP.distanceTo(new THREE.Vector2(0, -2 * radius)) <= 2.9 * radius ||
		inP.distanceTo(new THREE.Vector2(width + 2 * radius, -2 * radius)) <= 2.9 * radius ||
		inP.distanceTo(new THREE.Vector2(width + 2 * radius, height)) <= 2.9 * radius ||
		inP.distanceTo(new THREE.Vector2(0, height)) <= 2.9 * radius ||
		inP.distanceTo(new THREE.Vector2(width / 2 + radius, -2 * radius)) <= (3.5 * radius) / 2 ||
		inP.distanceTo(new THREE.Vector2(width / 2 + radius, height)) <= (3.5 * radius) / 2
	) {
		if (n == 0) {
	...
	// Ball collides with top or bottom
		...
		(inP.y >= height - radius || inP.y <= -radius) &&
		(inP.x < width / 2 + radius - (3.5 * radius) / 2 ||
			inP.x > width / 2 + radius + (3.5 * radius) / 2)
	) {
...
function ballCollision(v1, v2, p1, p2, prevPos1, prevPos2, n, k) {
	...
	const v2Copy = v2.clone()
	const p1Copy = p1.clone()
	const p2Copy = p2.clone()
	...
	if (p1Copy.distanceTo(p2Copy) < 2 * radius) {
		...
		outV1
			.subVectors(
				v1Copy,
				pDiff1.multiplyScalar(vDiff1.dot(pDiff1)).divideScalar(p1Copy.distanceToSquared(p2Copy))
			)
			.multiplyScalar(elast)
		outPos1 = prevPos1
		tau[n] = outV1.angle()
		...
		outV2
			.subVectors(
				v2Copy,
				pDiff2.multiplyScalar(vDiff2.dot(pDiff2)).divideScalar(p2Copy.distanceToSquared(p1Copy))
			)
			.multiplyScalar(elast)
		outPos2 = prevPos2
		tau[k] = outV2.angle()
...
export { edgeDetection, ballCollision, updateCounter, updateRowCounter }
